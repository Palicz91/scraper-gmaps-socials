...
Updated: now returns negative review stats + estimated totals.
"""
...
def run_single_place_audit(maps_url: str, place_id: str, place_name: str = "", place_address: str = "") -> dict | None:
    ...
    try:
        ...
        # Map all review fields including negative stats
        ...
        negative_total = int(place_data.get('negative_total', 0) or 0)
        negative_unanswered = int(place_data.get('negative_unanswered', 0) or 0)
        negative_unanswered_pct = float(place_data.get('negative_unanswered_pct', 0) or 0)
        est_unanswered = int(place_data.get('est_unanswered', 0) or 0)
        est_negative_unanswered = int(place_data.get('est_negative_unanswered', 0) or 0)
        ...
        result = {
            ...
            'negative_unanswered_pct': negative_unanswered_pct,
            'est_unanswered': est_unanswered,
            'est_negative_unanswered': est_negative_unanswered,
            'stars_5': int(place_data.get('stars_5', 0) or 0),
            'stars_4': int(place_data.get('stars_4', 0) or 0),
            'stars_3': int(place_data.get('stars_3', 0) or 0),
            'stars_2': int(place_data.get('stars_2', 0) or 0),
            'stars_1': int(place_data.get('stars_1', 0) or 0),
            'est_stars_5': int(place_data.get('est_stars_5', 0) or 0),
            'est_stars_4': int(place_data.get('est_stars_4', 0) or 0),
            'est_stars_3': int(place_data.get('est_stars_3', 0) or 0),
            'est_stars_2': int(place_data.get('est_stars_2', 0) or 0),
            'est_stars_1': int(place_data.get('est_stars_1', 0) or 0),
        ...
        logger.info(
            ...
            f"{negative_unanswered}/{negative_total} negative unanswered"
        )
        return result
